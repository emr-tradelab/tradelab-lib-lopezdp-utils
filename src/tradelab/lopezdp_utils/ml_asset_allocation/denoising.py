...
def _fit_kde(
    ...
    Args:
        ...
        x: Points at which to evaluate the PDF. If None, uses obs.
...
def _get_pca(matrix: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    Args:
        matrix: Symmetric matrix (correlation or covariance).

    Returns:
        Tuple of (eigenvalues as diagonal matrix, eigenvectors).
    """
...
def _corr2cov(corr: np.ndarray, std: np.ndarray) -> np.ndarray:
    ...
    Args:
        ...
        std: Standard deviations of assets.
...
def _err_pdfs(
    ...
    Args:
        var: Variance parameter being optimized.
        e_val: Empirical eigenvalues (1D array).
        q: Aspect ratio T/N.
        bwidth: Bandwidth for KDE.
        pts: Points for MP PDF evaluation.
...
def denoised_corr(
    ...
    # set noise eigenvalues to their average
...
def denoise_cov(cov0: np.ndarray, q: float, bwidth: float) -> np.ndarray:
    ...
    e_max0, var0 = find_max_eval(np.diag(e_val0), q, bwidth)
