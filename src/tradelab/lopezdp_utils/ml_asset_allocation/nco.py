...
Depends on ONC clustering from feature_importance/clustering.py.
...
def _min_var_port(cov: np.ndarray) -> np.ndarray:
    ...
    Returns:
        Weight vector as column array (N x 1).
    """
...
def _opt_port(cov: np.ndarray, mu: np.ndarray | None = None) -> np.ndarray:
    ...
    Args:
        cov: Covariance matrix.
        mu: Expected returns vector (N x 1). If None, min-variance.
...
def opt_port_nco(
    ...
    Final weights = intracluster weights × intercluster weights.
