...
def bagging_accuracy(n: int, p: float, k: int = 2) -> float:
    ...
    each with individual accuracy p, classifying into k classes.
...
def build_random_forest(
    ...
    Provides three configurations from AFML Snippet 6.2:
    ...
    - **Method 1**: ``BaggingClassifier`` wrapping ``DecisionTreeClassifier`` with
      ``max_samples`` set to average uniqueness.
    - **Method 2**: ``BaggingClassifier`` wrapping a single-tree
      ``RandomForestClassifier`` with ``max_samples`` set to average uniqueness.

    Args:
        avg_uniqueness: Average uniqueness of the dataset. Required for methods 1 and 2.
        n_estimators: Number of estimators in the ensemble.
        method: Configuration to use (0, 1, or 2).

    Returns:
        A fitted-ready scikit-learn classifier.
...
def bagging_classifier_factory(
    ...
    Factory for wrapping any base estimator in a BaggingClassifier for
    scalability and variance reduction.

    Args:
        base_estimator: The base estimator to bag.
