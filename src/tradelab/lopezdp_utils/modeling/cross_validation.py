"""Purging, embargoing, and cross-validation utilities for financial ML.

Implements time-aware sample removal and scoring to prevent information leakage
when cross-validating models on financial data with overlapping labels.
...
def get_embargo_times(
    ...
    against serial-correlation leakage that purging alone cannot address.
...
class PurgedKFold(KFold):
    ...
    Args:
        ...
        t1: pandas Series of label "through dates" (index = start, value = end).
            Required — raises ValueError if None or not a Series.
    ...
    def __init__(
        ...
        if not isinstance(t1, pd.Series):
            raise ValueError("t1 must be a pd.Series of label through-dates")
...
def cv_score(
    ...
    Drop-in replacement for scikit-learn's ``cross_val_score`` that fixes
    sample-weight routing and the ``log_loss`` classes bug.
    ...
    Args:
        ...
        pct_embargo: Embargo fraction (used if ``cv_gen`` is not provided).
...
def probability_weighted_accuracy(
    ...
    predictions more severely.

    Args:
        y_true: True labels, shape (n_samples,).
        y_pred: Predicted labels, shape (n_samples,).
        prob: Maximum predicted probability for each observation, shape (n_samples,).
        n_classes: Number of classes (default 2).

    Returns:
        Probability-weighted accuracy score in [-1, 1].
